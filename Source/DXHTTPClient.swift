...
public class DXHTTPClient {
    ...
    static var environment = DXEnvironment.sandbox
    static var clientId = ""
    ...
    static var isAuthenticated: Bool {
        return DXHTTPClient.accessToken != nil && DXHTTPClient.refreshToken != nil
    }
    ...
    //  Tries to create a DXError from response, otherwises returns a default error
    func defaultFailureHandler(
        error: Error?,
        response: HTTPURLResponse
    ) -> Error
    {
        return DXError.API.createError(from: response)
            ?? error
            ?? DXError.API.requestFailedForUnknownReason(message: "Request returned without response.")
    }
    ...
    ) -> Void
    {
        ...
        DXHTTPClient.lock.lock(); defer { DXHTTPClient.lock.unlock() }
        ...
        if DXHTTPClient.isRefreshing {
            DXHTTPClient.requestsToRetry.append {
            ...
            let requestCreator = createRequest ?? standardAuthRequestCreator(accessToken: DXHTTPClient.accessToken)
            ...
            HTTPClient.perform(urlRequest) { error, response, data in
                ...
                if error != nil || (response?.statusCode != nil && response!.statusCode > 299) {
                    ...
                    if DXError.API.ErrorCode.errorCode(for: response) == .tokenRefreshRequired {
                        DXHTTPClient.lock.lock(); defer { DXHTTPClient.lock.unlock() }
                        ...
                        if !DXHTTPClient.isRefreshing {
                            DXHTTPClient.isRefreshing = true
                    ...
                    guard let response = response, let data = data else {
                        completionWithAnyResult?(DXError.API.requestFailedForUnknownReason(message: "Request returned without response."), nil); return
                    }
