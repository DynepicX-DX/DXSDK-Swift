...
public final class PlayPortalClassClient: PlayPortalHTTPClient {
  
  public static let shared = PlayPortalClassClient()
  ...
  public func createClass(
    name: String,
    courseId: String? = nil,
    _ completion: @escaping (_ error: Error?, _ class: PlayPortalClass?) -> Void)
    -> Void
  {
    let body: [String: Any] = [
      "name": name,
  ...
  public func getClasses(
    page: Int? = nil,
    limit: Int? = nil,
    _ completion: @escaping (_ error: Error?, _ classes: [PlayPortalClass]?) -> Void)
    -> Void
  {
    ...
    let handleSuccess: HandleSuccess<[PlayPortalClass]> = { response, data in
      guard let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
        let docs = json["docs"] else {
          throw PlayPortalError.API.unableToDeserializeResult(message: "Unable to deserialize PlayPortalClass array.")
      }
