...
public final class DXLeaderboardClient: DXHTTPClient {
    ...
    -> Void
  {
    ...
    let handleSuccess: HandleSuccess<[DXLeaderboardEntry]> = { response, data in
      guard let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
        let docs = json["docs"] else {
          throw DXError.API.unableToDeserializeResult(message: "Unable to deserialize DXLeaderboardEntry array.")
      }
