...
public class DXNotificationClient: DXHTTPClient {
  ...
  private func register(
    ...
    -> Void
  {
    assert(DXNotificationClient.refreshToken != nil, "User must be logged in before registering for push notifications.")
    
    let body = [
      "refreshToken": DXNotificationClient.refreshToken,
    ...
    -> Void
  {
    ...
    let handleSuccess: HandleSuccess<[DXNotification]> = { response, data in
      guard let json = (try JSONSerialization.jsonObject(with: data, options: [])) as? [String: Any]
        , let docs = json["docs"]
        else {
          throw DXError.API.unableToDeserializeResult(message: "Unable to deserialize DXNotification array.")
      }
