...
public final class DXDataClient: DXHTTPClient {
    ...
    -> Void
  {
    ...
    request(
      ...
      completionWithNoResult: { error in
        if let error = error as? DXError.API
          , case DXError.API.requestFailed(.alreadyExists, _) = error
        {
          completion?(nil)
        } else {
          completion?(error)
    ...
    let handleSuccess: HandleSuccess<Any> = { response, data in
      guard let json = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
        let data = json?["data"]
          ...
          throw DXError.API.unableToDeserializeResult(message: "Couldn't retrieve 'data' from result.")
      }
      return data
    }
  ...
  /**
   ...
   - Parameter completion: The closure called when the request finishes.
   - Parameter error: The error returned for an unsuccessful request.
   - Parameter bucket: A `DXDataBucket` instance containing the data at `atKey` returned for a successful request.
   - Returns: Void
   */
  public func get(
    bucketNamed bucketName: String,
    ...
    -> Void
  {
    get(bucketNamed: bucketName) { error, bucket in
      if let error = error {
        ...
        completion?(DXError.API.requestFailedForUnknownReason(message: "Bucket was not of expected type."), nil)
      }
    }
  }
